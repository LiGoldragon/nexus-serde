//! Deserializer: parse nexus text into types implementing [`serde::Deserialize`].
//!
...
//! Sentinel-name dispatch on `@NexusBind` / `@NexusMutate` / `@NexusNegate`
//! newtype-struct names handles the three query-layer wrappers.
...
impl<'de> de::Deserializer<'de> for &mut Deserializer<'de> {
    ...
    fn deserialize_char<V: Visitor<'de>>(self, visitor: V) -> Result<V::Value> {
        // Mirrors nota-serde: accepts both Token::Str and Token::Ident
        // to match the bare-string serialisation of single-char idents.
