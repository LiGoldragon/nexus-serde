//! Serializer emitting canonical nexus text — superset of nota.
//!
//! Inherits all of nota's canonical rules (source-declaration field
//! order, sorted map keys, shortest-roundtrip numbers, single-space
//! separators, `#`-prefixed hex bytes, `[ ]` / `[| |]` strings) and
//! adds sentinel-name dispatch on three wrapper types: `Bind` → `@x`,
//! `Mutate<T>` → `~value`, `Negate<T>` → `!value`. Sigils sit
//! immediately before their target token with no intervening
//! whitespace.
