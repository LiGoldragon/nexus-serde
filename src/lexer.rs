//! Tokenizer for nexus text. Superset of the nota lexer — adds tokens
//! for `~`, `@`, `!`, `{`, `}`, and the compound pipe-delimiters
//! `(|` / `|)`, `{|` / `|}`.
//!
    ...
    pub fn next_token(&mut self) -> Result<Option<Token>> {
        ...
        match b {
            ...
            b'{' => {
                ...
                if self.peek_byte() == Some(b'|') {
                    self.pos += 1;
                    Ok(Some(Token::LBracePipe))
                } else {
                    Ok(Some(Token::LBrace))
                }
            }
            b'}' => { self.pos += 1; Ok(Some(Token::RBrace)) }
            b'|' => {
                self.pos += 1;
                match self.peek_byte() {
                    Some(b')') => { self.pos += 1; Ok(Some(Token::RParenPipe)) }
                    Some(b'}') => { self.pos += 1; Ok(Some(Token::RBracePipe)) }
            ...
            b'~' => { self.pos += 1; Ok(Some(Token::Tilde)) }
            b'@' => { self.pos += 1; Ok(Some(Token::At)) }
            b'!' => { self.pos += 1; Ok(Some(Token::Bang)) }
            b'[' => {
                self.pos += 1;
                if self.peek_byte() == Some(b'|') {
                    self.pos += 1;
